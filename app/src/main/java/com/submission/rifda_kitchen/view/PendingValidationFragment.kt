...
import com.submission.rifda_kitchen.databinding.FragmentPendingValidationBinding
...
class PendingValidationFragment : Fragment() {
    private var _binding: FragmentPendingValidationBinding? = null
    private val binding get() = _binding!!
    private val repository = Repository()
    private val historyViewModel: HistoryViewmodel by viewModels { ViewmodelFactory(repository) }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentPendingValidationBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun observeViewModel() {
        historyViewModel.orderList.observe(viewLifecycleOwner) { orders ->
            ...
            val filteredOrders = orders?.filter { it.orderStatus == "Menunggu Konfirmasi" }
