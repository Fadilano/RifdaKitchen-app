...
import com.submission.rifda_kitchen.databinding.FragmentAwaitingPaymentBinding
...
class AwaitingPaymentFragment : Fragment() {
    private var _binding: FragmentAwaitingPaymentBinding? = null
    private val binding get() = _binding!!
    private val repository = Repository()
    private val historyViewModel: HistoryViewmodel by viewModels { ViewmodelFactory(repository) }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentAwaitingPaymentBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun observeViewModel() {
        historyViewModel.orderList.observe(viewLifecycleOwner) { orders ->
            ...
            val filteredOrders = orders?.filter { it.orderStatus == "Pesanan Dikonfirmasi, silahkan lakukan pembayaran" }
